...
describe('fromTmdbResponse', () => {
  ...
  describe('From Details', () => {
    ...
    it('should assign all fields correctly', () => {
      ...
      expect(response.backdropPath).toBe('/2FonLz0RPxbBriOlZ9mWhYdlqCp.jpg')
      ...
      expect(response.title).toBe('Clueless')
      expect(response.video).toBe(false)
      expect(response.voteAverage).toBe(7.282)
      expect(response.voteCount).toBe(3953)
      expect(response.crew).toEqual([
